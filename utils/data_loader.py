...
class TrajDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # return self.data.loc[index].wgs_seq
    def __len__(self):
...
# Load traj dataset for trajcluster learning
def read_trajcluster_traj_dataset(file_path):

    logging.info('[Load trajclutser traj dataset] START.')
    _time = time.time()

    df_trajs = pd.read_pickle(file_path)
    l = df_trajs.shape[0]
    # train_idx = (int(l * 0), 1280)
    train_idx = (int(l * 0), l)
    eval_idx = (int(l * 0.7), int(l * 0.8))
    test_idx = (int(l * 0.8), int(l * 1.0))
